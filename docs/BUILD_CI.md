...
- 常见问题
  ...
  - 测试失败：先运行 `procvision-cli validate ./algorithm-example --full --tail-logs` 自查接口实现
...
- 本地运行：
  - `procvision-cli run ./algorithm-example --cur-image ./cur.jpg --guide-image ./guide.jpg --step-desc "Step 1" --guide-info @guide.json`
- 说明：
  - 本地运行会将双图写入共享内存，并通过适配器协议调用一次 `execute`
...
- 运行失败：确保图片路径存在；`--guide-info` 为合法 JSON 或 `@file.json`
